...
TEST_F(SegLRUCacheMultiThreadTest, ConcurrentInsertAndFind) {
    ...
    EXPECT_EQ(cache.Size(), static_cast<size_t>(num_threads * items_per_thread)); // Size should remain the same
}
...
TEST_F(SegLRUCacheMultiThreadTest, ConcurrentMixedOperations) {
    ...
    const size_t capacity_per_segment = 100; // Smaller capacity to force evictions: 100 * 16 = 1600 total
                                             // Total operations will be high, many inserts/removals
    ...
    for (int i = 0; i < num_threads; ++i) {
        threads.emplace_back([&, i]() { // Capture by reference, i by value
            for (int j = 0; j < ops_per_thread; ++j) {
                KeyType key = static_cast<KeyType>((i * ops_per_thread + j) % (num_threads * ops_per_thread / 2)); // Keys will overlap
                int op_type = j % 3; // Simple way to alternate operations
                ...
                if (op_type == 0) { // Insert
                    ...
                    ValueType retrieved_value;
                    if (cache.Find(key, retrieved_value)) {
                        // Could verify value if we knew it was inserted and not evicted/removed
                        successful_finds++;
                    }
                } else { // Remove
                    if (cache.Remove(key)) {
    ...
    // Basic sanity checks for a stress test - not exact counts due to concurrency and evictions
    std::cout << "Mixed Ops Test - Successful Inserts: " << successful_inserts.load() << std::endl;
    std::cout << "Mixed Ops Test - Successful Finds: " << successful_finds.load() << std::endl;
    std::cout << "Mixed Ops Test - Successful Removes: " << successful_removes.load() << std::endl;
    std::cout << "Mixed Ops Test - Final Cache Size: " << cache.Size() << std::endl;

    // Expect that some operations succeeded. The exact numbers are hard to predict.
    EXPECT_GT(successful_inserts.load(), 0); 
    // Finds and removes might be 0 if items are immediately evicted or removed by other threads.
    // // The most important thing is that it doesn't crash or deadlock.
    // EXPECT_LE(cache.Size(), capacity_per_segment * SegLRUCache<KeyType, ValueType>::segNum);
}
